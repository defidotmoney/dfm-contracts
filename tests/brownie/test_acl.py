...
def test_set_market_hooks(controller, market, alice):
    with brownie.reverts("MainController: Only owner"):
        controller.set_market_hooks(market, ZERO_ADDRESS, 0, {"from": alice})
...
def test_set_amm_fee(market, alice):
    with brownie.reverts("MarketOperator: Only owner"):
...
def test_create_loan(market, alice):
    with brownie.reverts("MarketOperator: Only controller"):
...
def test_deposit_range(amm, alice):
    with brownie.reverts("AMM: Only operator"):
