...
class StatefulLendBorrow(RuleBasedStateMachine):
    ...
    amount = st.integers(min_value=0, max_value=2**255 - 1)
    c_amount = st.integers(min_value=0, max_value=2**255 - 1)
    user_id = st.integers(min_value=0, max_value=9)
    t = st.integers(min_value=0, max_value=86400 * 365)
    rate = st.integers(min_value=0, max_value=2**32)  # Negative is probably not good btw
    ...
    def repay(self, amount, user_id):
        ...
        with boa.env.prank(user):
            if amount == 0:
                with boa.reverts():
                    self.controller.adjust_loan(user, self.market, 0, 0)
                return
            ...
            self.controller.collect_fees([self.market.address])
            # And we need to transfer them to us if necessary
            user_debt = self.market.debt(user)
            user_balance = self.stablecoin.balanceOf(user)
            diff = user_debt - user_balance
            if diff > 0:
                admin_balance = self.stablecoin.balanceOf(self.fee_receiver)
                with boa.env.prank(self.fee_receiver):
    ...
    def sum_of_debts(self):
        assert abs(
            sum(self.market.debt(u) for u in self.accounts) - self.market.total_debt()
        ) <= len(self.accounts)
    ...
    def debt_payable(self):
        with boa.env.prank(self.admin):
            ...
            # b = self.stablecoin.balanceOf(self.fee_receiver)
...
def test_rate_too_high(
    ...
    with boa.env.anchor():
        ...
        state.change_rate(
            rate=19298681539552733520784193015473224553355594960504706685844695763378761203935
        )
...
def test_overflow(
    ...
    state.sum_of_debts()
    state.borrow_more(
        amount=102598604287098624639570500830661495567, c_amount=12171265979771193868, user_id=0
    )
    state.debt_payable()
    state.sum_of_debts()
