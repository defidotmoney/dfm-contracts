...
class AdiabaticTrader(RuleBasedStateMachine):
    ...
    def initializer(self, collateral_amount, n):
        ...
        with boa.env.prank(user):
            ...
            loan_amount = self.market.max_borrowable(collateral_amount, n)
            self.controller.create_loan(user, self.market, collateral_amount, loan_amount, n)
    ...
    def random_trade(self, amount_fraction, is_pump):
        ...
        with boa.env.prank(user):
            if is_pump:
                amount = min(
                    int(self.loan_amount * amount_fraction), self.stablecoin.balanceOf(user)
                )
                self.amm.exchange(0, 1, amount, 0)
            else:
    ...
    def health(self):
        if not self.not_enough:
            assert self.market.health(self.accounts[0]) > 0
