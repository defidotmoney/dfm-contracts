...
def test_n1(amm, market, collateral, debt, n):
    n0 = amm.active_band()
    A = amm.A()
    p0 = amm.p_oracle_down(n0) / 1e18
    discounted_collateral = collateral * (10**18 - market.loan_discount()) // 10**18

    too_high = False
    too_deep = False
    try:
        n1 = market.calculate_debt_n1(collateral, debt, n)
    ...
    assert discounted_collateral * amm.p_oracle_up(n1) / 1e18 >= debt
    if n2 < 1023:
        assert discounted_collateral * amm.p_oracle_down(n2) / 1e18 <= debt
