...
def test_min_coin_amount(
    swaps, initial_amounts, alice, peg_keepers, pk_regulator, peg_keeper_updater
):
...
def test_almost_balanced(
    swaps, alice, peg_keepers, pk_regulator, peg_keeper_updater, redeemable_tokens, stablecoin
):
    for swap, peg_keeper, rtoken in zip(swaps, peg_keepers, redeemable_tokens):
        with boa.env.prank(alice):
            diff = swap.balances(1) * 10 ** (18 - stablecoin.decimals()) - swap.balances(
                0
            ) * 10 ** (18 - rtoken.decimals())
