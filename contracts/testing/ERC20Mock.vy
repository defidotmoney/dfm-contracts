...
@external
def setMinter(minter: address, is_approved: bool):
    assert msg.sender == self.owner
    self.isMinter[minter] = is_approved


@external
def mint(_target: address, _value: uint256) -> bool:
    assert self.isMinter[msg.sender], "Not minter"
    self.total_supply += _value
    self.balanceOf[_target] += _value
    log Transfer(empty(address), _target, _value)
...
def burn(_target: address, _value: uint256) -> bool:
    ...
    self.total_supply -= _value
    self.balanceOf[_target] -= _value
    log Transfer(_target, empty(address), _value)
