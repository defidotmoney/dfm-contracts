...
"""
@title Test CurveStableSwapNG - NOT FOR USE IN PRODUCTION
@author Curve.Fi
@license Copyright (c) Curve.Fi, 2020-2023 - all rights reserved
@dev Some functionality is modified / removed, under no circumstances
     should this contract be used with real funds.
"""
...
rate_multipliers: DynArray[uint256, MAX_COINS]
...
def _stored_rates() -> DynArray[uint256, MAX_COINS]:
    ...
    @dev If the coin has a rate oracle that has been properly initialised,
         ...
         contract.
    """
    rates: DynArray[uint256, MAX_COINS] = self.rate_multipliers
...
def _withdraw_admin_fees():
    fee_receiver: address = msg.sender
...
@external
def set_new_fee(_new_fee: uint256):

    # set new fee:
    assert _new_fee <= MAX_FEE
    self.fee = _new_fee

    log ApplyNewFee(_new_fee, 0)
